...
def frame():
    ...
    # tk.Button(window, text='check_book', font=('Microsoft YaHei', 15), width=10, height=2, command=search_book).place(
    #     x=350, y=150)
...
#                          )
#     cursor = db.cursor()
#     sql = "DELETE FROM book where datediff(now(), date) > 3"
#     cursor.execute(sql)
#     db.commit()
#     db.close()
#     msg.showinfo(title='Success！', message='Delete bookings older than three days!')
...
def cancel():  # 撤销图书
    ...
    lable1 = tk.Label(win, text='Please fill in the information of the delete equipment :',
